...
class Model(nn.Module):
    def __init__(self):                      # on init
        ...
        self.dense1 = nn.Linear(625, 50)    # create dense layer
        self.dense2 = nn.Linear(50, 10)     # create dense layer

        # self.decode = nn.Linear(10, 784)   # for autoencoder

    def forward(self, x):              # send data through neural net
        # send x through first convolution (shrinks x a bit)
        x = self.conv1(x)
        x = F.relu(x)                  # activation function
        # send x through second convolution (shrinks x a bit more)
        x = self.conv2(x)
